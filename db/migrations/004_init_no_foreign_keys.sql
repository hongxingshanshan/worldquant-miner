-- Alpha 数据库初始化脚本（无外键版本）
-- 创建时间: 2026-05-20
-- 说明: 移除外键约束，由应用层保证数据完整性
...
CREATE INDEX IF NOT EXISTS idx_alpha_date_modified ON alpha(date_modified);
CREATE INDEX IF NOT EXISTS idx_alpha_stage ON alpha(stage);
CREATE INDEX IF NOT EXISTS idx_alpha_status ON alpha(status);
CREATE INDEX IF NOT EXISTS idx_alpha_grade ON alpha(grade);
CREATE INDEX IF NOT EXISTS idx_perf_sharpe ON alpha_performance(sharpe);
CREATE INDEX IF NOT EXISTS idx_perf_fitness ON alpha_performance(fitness);
CREATE INDEX IF NOT EXISTS idx_perf_stage ON alpha_performance(stage);